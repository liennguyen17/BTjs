    ...
    return http.readPosts().then((res) => {
        ...
        const html = res.reduce((result, current) => {
          ...
          <div>
            ...
            <button class="btn btn-danger btn-start-remove" data-id="${current.id}">Remove</button>
          </div>
        </div>
            `
            return result
...
const startEdit = id => {
    ...
    const remove = id => {
        ...
        return renderAll()
            ...
            alert('Remove Sussessfully', 'danger')
        })
    }
...
const alert = (message, type = 'success') => {
    ...
    const init = () => {
        ...
        document.getElementById('list').addEventListener('click', (event) => {
            if(event.target.classList.contains('btn-start-edit')) {
                const id = event.target.dataset.id
                const name = event.target.parentElement.previouElementSibling.firstElementChild.textContent
